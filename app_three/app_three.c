...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			hImageList = ImageList_Create(16, 16, ILC_COLOR16 | ILC_MASK, 1, 0);
					
//#ifdef PIC_RESOURCE_USED
			//hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_COLOR3));
//#else
			hBitmap = (HBITMAP)LoadImage(hImageList, "1.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE); //¼ÓÔØÒ»×éÍ¼Æ¬
//#endif
			ImageList_AddMasked(hImageList, hBitmap, RGB(255, 255, 255));
			/**/

			tbb[3].iBitmap = hBitmap;
			tbb[3].fsState = TBSTATE_ENABLED;
			tbb[3].fsStyle = TBSTYLE_BUTTON;
			tbb[3].idCommand = ID_FILE_COPY;
