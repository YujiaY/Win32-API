...
HWND CreateToolbar(HWND hParentWnd)
{
	HWND hWndTB;
	TBBUTTON tbb[3];
	HIMAGELIST  hHotImageList, hDisableImageList;
	HBITMAP hBitmap;
	...
	HIMAGELIST  hImageList = ImageList_Create(16, 16, ILC_COLOR24 | ILC_MASK, 3, 1);


	/*
	int iBitmap[3] = { 0 };
	// WORD Id[13] = { IDB_FileManage, IDB_ScreenControl, IDB_Cmd };

	iBitmap[0] = ImageList_Add(hImageList, LoadBitmap(hInst,"1.bmp"), 0);
	iBitmap[1] = ImageList_Add(hImageList, LoadBitmap(hInst, "2.bmp"), 0);
	iBitmap[2] = ImageList_Add(hImageList, LoadBitmap(hInst, "3.bmp"), 0);
	...
	*/
...
#else
	hBitmap = (HBITMAP)LoadImage(NULL, TEXT("1.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION); //加载一组图片
#endif
	ImageList_AddMasked(hImageList, hBitmap, RGB(255, 255, 255));
	DeleteObject(hBitmap);
	// SendMessage(hWndTB, TB_SETIMAGELIST, 0, (LPARAM)hImageList[0]); //正常显示时的图像列表
	// +1s
	hBitmap = (HBITMAP)LoadImage(NULL, TEXT("2.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	ImageList_AddMasked(hImageList, hBitmap, RGB(255, 255, 255));
	DeleteObject(hBitmap);
	// SendMessage(hWndTB, TB_SETIMAGELIST, 0, (LPARAM)hImageList[1]);
	//+2s
	hBitmap = (HBITMAP)LoadImage(NULL, TEXT("3.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	ImageList_AddMasked(hImageList, hBitmap, RGB(255, 255, 255));
	DeleteObject(hBitmap);
	SendMessage(hWndTB, TB_SETIMAGELIST, 0, (LPARAM)hImageList);
	...
	// Start of HotImageList
	hHotImageList = ImageList_Create(16, 16, ILC_COLOR24 | ILC_MASK, 3, 1);
#ifdef PIC_RESOURCE_USED
	hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_GREEN3));
#else
	hBitmap = (HBITMAP)LoadImage(NULL, TEXT("8.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
#endif
	ImageList_AddMasked(hHotImageList, hBitmap, RGB(0, 0, 0));
	ImageList_AddMasked(hHotImageList, hBitmap, RGB(0, 0, 0));
	ImageList_AddMasked(hHotImageList, hBitmap, RGB(0, 0, 0));
	...
	hDisableImageList = ImageList_Create(16, 16, ILC_COLOR24 | ILC_MASK, 3, 1);
#ifdef PIC_RESOURCE_USED
	hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_GRAY3));
#else
	hBitmap = (HBITMAP)LoadImage(NULL, TEXT("9.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
#endif
	ImageList_AddMasked(hDisableImageList, hBitmap, RGB(0, 0, 0));
	ImageList_AddMasked(hDisableImageList, hBitmap, RGB(0, 0, 0));
	ImageList_AddMasked(hDisableImageList, hBitmap, RGB(0, 0, 0));
	...
	tbb[2].fsState = TBSTATE_ENABLED; //TBSTATE_INDETERMINATE;
...
{
#define PANEL_NUM 4
	int array[PANEL_NUM] = { 80,80 * 2, 80 * 3, -1 };
	HINSTANCE hInst = GetModuleHandle(NULL);
	//创建Statusbar控件
	...
	if (hWndStatus)
	{
		...
		SendMessage(hWndStatus, SB_SETTEXT, (LPARAM)0, (WPARAM)TEXT("panel-aadd1")); //设置第二个面板内容
		SendMessage(hWndStatus, SB_SETTEXT, (LPARAM)1, (WPARAM)TEXT("panel-sdss2")); //设置第三个面板内容
		SendMessage(hWndStatus, SB_SETTEXT, (LPARAM)2, (WPARAM)TEXT("panel-3")); //设置第三个面板内容
	}
#undef PANEL_NUM
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		SendMessage(hStatusbar, SB_SETTEXT, 3, (LPARAM)(LPSTR)szBuf);
	}
	return 0;
